...
export class ExpenseService {
  ...
  async updateExpense(
    ...
    if (request.amount !== existingExpense.amount) {
      ...
      if (delta < 0) {
        ...
        await this.transactionService.createCreditTransaction(userId, txReq);
      } else {
        await this.transactionService.createDebitTransaction(userId, txReq);
      }
  ...
  async findByUuid(userId: string, uuid: string): Promise<ExpenseDetailDto> {
  ...
  async deleteExpense(userId: string, uuid: string): Promise<void> {
    const expense = await this.repository.findOneBy({ userId, uuid });
    if (expense) {
      const txReq = {
      ...
      void (await this.transactionService.createCreditTransaction(
        userId,
        txReq,
      ));
  ...
  async getExpensesByBudgetId(
    ...
    return this.repository
      .findExpensesByBudgetId(userId, budgetId, page, size, [
        'category',
        'account',
        'budget',
      ])
